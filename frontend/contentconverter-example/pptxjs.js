...
(function ($) {
    $.fn.pptxToHtml = function (options) {
        ...
        function convertToHtml(file) {
            ...
            // Fire the afterRender callback if provided (used by contentConverter service)
        ...
        function getSlideSizeAndSetDefaultTextStyle(zip) {
            ...
            slideWidth = (sldSzWidth * slideFactor + settings.incSlide.width) | 0;
            slideHeight = (sldSzHeight * slideFactor + settings.incSlide.height) | 0;
            console.log("[SLIDE SIZE] EMU: " + sldSzWidth + "x" + sldSzHeight + " → px: " + slideWidth + "x" + slideHeight + " (slideFactor=" + slideFactor + ")");
            rtenObj = {
                "width": slideWidth,
                "height": slideHeight
            };
        ...
        function processSingleSlide(zip, sldFileName, index, slideSize) {
            ...
            */
                ...
                for (var i = 0; i < RelationshipArray.length; i++) {
                    switch (RelationshipArray[i]["attrs"]["Type"]) {
                        ...
                        case "http://schemas.openxmlformats.org/officeDocument/2006/relationships/themeOverride":
                            themeOverrideFilename = RelationshipArray[i]["attrs"]["Target"].replace("../", "ppt/");
                            break;
                        default:
            ...
            if (themeOverrideFilename) {
                console.log("[THEME OVERRIDE] layout=" + layoutFilename + " themeOverride=" + themeOverrideFilename);
            }
            ...
            // Extract font families from theme and load them via Google Fonts
            if (themeContent) {
                var fontScheme = getTextByPathList(themeContent, ["a:theme", "a:themeElements", "a:fontScheme"]);
                var themeFonts = [];
                var majorFont = getTextByPathList(fontScheme, ["a:majorFont", "a:latin", "attrs", "typeface"]);
                var minorFont = getTextByPathList(fontScheme, ["a:minorFont", "a:latin", "attrs", "typeface"]);
                if (majorFont) themeFonts.push(majorFont);
                if (minorFont && minorFont !== majorFont) themeFonts.push(minorFont);
                console.log("[THEME FONTS] major=" + majorFont + " minor=" + minorFont);
                if (themeFonts.length > 0) {
                    // Also scan for explicit fonts used in slides
                    var linkId = "pptxjs-google-fonts";
                    if (!document.getElementById(linkId)) {
                        var families = themeFonts.map(function(f) { return f.replace(/ /g, "+") + ":wght@400;700"; }).join("&family=");
                        var linkEl = document.createElement("link");
                        linkEl.id = linkId;
                        linkEl.rel = "stylesheet";
                        linkEl.href = "https://fonts.googleapis.com/css2?family=" + families + "&display=swap";
                        document.head.appendChild(linkEl);
                        console.log("[GOOGLE FONTS] Loading: " + linkEl.href);
                    }
                }
            }
            //Load diagram file
        ...
        */
        function processPicNode(node, warpObj, source, sType) {
            ...
            // Guard: skip external/linked images where the relationship target is missing
            if (!resObj || !resObj[rid] || !resObj[rid]["target"]) {
                return "";
            }
            ...
            var zip = warpObj["zip"];
            var zipEntry = zip.file(imgName);
            if (!zipEntry) { return ""; }
            var imgArrayBuffer = zipEntry.asArrayBuffer();
            var mimeType = "";
            ...
            if (vdoNode !== undefined & mediaProcess) {
                // Embedded videos use r:embed; externally linked use r:link — try both
                vdoRid = vdoNode["attrs"]["r:embed"] || vdoNode["attrs"]["r:link"];
                if (vdoRid && resObj[vdoRid]) {
                ...
                if (checkIfLink) {
                    ...
                    var vdoZipEntry = zip.file(vdoFile);
                    if (vdoZipEntry && (vdoFileExt == "mp4" || vdoFileExt == "webm" || vdoFileExt == "ogg" || vdoFileExt == "wmv" || vdoFileExt == "avi" || vdoFileExt == "mov")) {
                        uInt8Array = vdoZipEntry.asArrayBuffer();
            ...
            if ((vdoNode === undefined && audioNode === undefined) || !mediaProcess || !mediaSupportFlag) {
                ...
                if (vdoNode !== undefined && !isVdeoLink) {
                    var canPlayNatively = (vdoFileExt == "mp4" || vdoFileExt == "webm" || vdoFileExt == "ogg");
                    if (canPlayNatively) {
                        rtrnData += "<video src='" + vdoBlob + "' controls style='width: 100%; height: 100%'>Your browser does not support the video tag.</video>";
                    } else {
                        rtrnData += "<div style='width:100%;height:100%;display:flex;flex-direction:column;align-items:center;justify-content:center;background:#000;color:#fff;font-size:14px;gap:8px'>" +
                            "<span>&#127909; " + (vdoFileExt ? vdoFileExt.toUpperCase() : "Video") + " (not playable in browser)</span>" +
                            "<a href='" + vdoBlob + "' download='video." + vdoFileExt + "' style='color:#90caf9;font-size:12px'>Download video</a>" +
                            "</div>";
                    }
                } else if (vdoNode !== undefined && isVdeoLink) {
                    rtrnData += "<iframe src='" + toEmbedUrl(vdoFile) + "' frameborder='0' allow='autoplay; encrypted-media' allowfullscreen style='width: 100%; height: 100%'></iframe>";
                }
        ...
        function genTextBody(textBodyNode, spNode, slideLayoutSpNode, slideMasterSpNode, type, idx, warpObj, tbl_col_width) {
            ...
            for (var i = 0; i < apNode.length; i++) {
                ...
                // Subtract body insets from paragraph container width to match PowerPoint text area
                var _bodyPrAttrs = getTextByPathList(spNode, ["p:txBody", "a:bodyPr", "attrs"]) || {};
                var _lIns = (_bodyPrAttrs["lIns"] !== undefined) ? parseInt(_bodyPrAttrs["lIns"]) : 91440;
                var _rIns = (_bodyPrAttrs["rIns"] !== undefined) ? parseInt(_bodyPrAttrs["rIns"]) : 91440;
                var _bodyInsetPx = (_lIns + _rIns) * slideFactor;
                var _cxPx = prg_width_node !== undefined ? parseInt(prg_width_node) * slideFactor - _bodyInsetPx : undefined;
                var sld_prg_width = (_cxPx !== undefined) ? ("width:" + _cxPx + "px;") : "width:inherit;";
                ...
                // bu_width = buText_ary[1] (bullet left offset = max(0, marL+indent))
                //          + buText_ary[2] (bullet char font size in px)
                // PowerPoint text area = cx - lIns - rIns - marL (body insets + paragraph margin)
                var bodyPrAttrs = getTextByPathList(textBodyNode, ["a:bodyPr", "attrs"]) || {};
                var lIns = (bodyPrAttrs["lIns"] !== undefined) ? parseInt(bodyPrAttrs["lIns"]) : 91440;
                var rIns = (bodyPrAttrs["rIns"] !== undefined) ? parseInt(bodyPrAttrs["rIns"]) : 91440;
                var bodyInsetPx = (lIns + rIns) * slideFactor;
                var bu_char_width = (isBullate && buText_ary[2] !== undefined && buText_ary[2] !== null) ? parseInt(buText_ary[2]) : 0;
                prg_width_node = parseInt(prg_width_node) * slideFactor - bu_width - mrgin_val + bu_char_width - bodyInsetPx;
                if (isBullate) {
        ...
        function genSpanElement(node, rIndex, pNode, textBodyNode, pFontStyle, slideLayoutSpNode, idx, type, rNodeLength, warpObj, isBullate) {
            ...
            if (linkID !== undefined && linkID != "") {
                ...
                return openElemnt + " class='text-block " + cssName + "' style='" + text_style + "'><a href='" + linkURL + "' " + linkColorSyle + "  " + linkTooltip + " target='_blank'>" +
                        text.replace(/\t/g, '&nbsp;&nbsp;&nbsp;&nbsp;').replace(/ {2,}/g, function(m){ return m.replace(/ /g, '&nbsp;'); }) + "</a>" + closeElemnt;
            } else {
                return openElemnt + " class='text-block " + cssName + "' style='" + text_style + "'>" + text.replace(/\t/g, '&nbsp;&nbsp;&nbsp;&nbsp;').replace(/ {2,}/g, function(m){ return m.replace(/ /g, '&nbsp;'); }) + closeElemnt;//"</bdi>";
            }
        ...
        function getSize(slideSpNode, slideLayoutSpNode, slideMasterSpNode) {
            ...
            if (ext === undefined) {
                ...
                // Connectors/lines can have cx=0 (vertical) or cy=0 (horizontal).
                // Give them at least 1px so borders/lines remain visible.
                if (w === 0 && h > 0) w = 1;
                if (h === 0 && w > 0) h = 1;
        ...
        function getVerticalMargins(pNode, textBodyNode, type, idx, warpObj) {
            ...
            // 1. Read from the paragraph's own pPr (highest priority)
            var spcBefNode = getTextByPathList(pNode, ["a:pPr", "a:spcBef", "a:spcPts", "attrs", "val"]);
            var spcBefPctNode = (spcBefNode === undefined) ? getTextByPathList(pNode, ["a:pPr", "a:spcBef", "a:spcPct", "attrs", "val"]) : undefined;
            var spcAftNode = getTextByPathList(pNode, ["a:pPr", "a:spcAft", "a:spcPts", "attrs", "val"]);
            var spcAftPctNode = (spcAftNode === undefined) ? getTextByPathList(pNode, ["a:pPr", "a:spcAft", "a:spcPct", "attrs", "val"]) : undefined;
            ...
            // 2. Fall back to textBodyNode's lstStyle and defPPr (applies to all types including shape/textBox)
            if (spcBefNode === undefined || spcAftNode === undefined || lnSpcNode === undefined) {
                var txBodyPPr = getTextByPathList(textBodyNode, ["a:lstStyle", lvlStr]);
                if (txBodyPPr === undefined) {
                    txBodyPPr = getTextByPathList(textBodyNode, ["a:lstStyle", "a:defPPr"]);
                }
                if (txBodyPPr === undefined) {
                    txBodyPPr = getTextByPathList(textBodyNode, ["a:defPPr"]);
                }
                if (txBodyPPr !== undefined) {
                    if (spcBefNode === undefined) {
                        spcBefNode = getTextByPathList(txBodyPPr, ["a:spcBef", "a:spcPts", "attrs", "val"]);
                        if (spcBefNode === undefined) {
                            spcBefPctNode = getTextByPathList(txBodyPPr, ["a:spcBef", "a:spcPct", "attrs", "val"]);
                        }
                    }
                    if (spcAftNode === undefined) {
                        spcAftNode = getTextByPathList(txBodyPPr, ["a:spcAft", "a:spcPts", "attrs", "val"]);
                        if (spcAftNode === undefined) {
                            spcAftPctNode = getTextByPathList(txBodyPPr, ["a:spcAft", "a:spcPct", "attrs", "val"]);
                        }
                    }
                    if (lnSpcNode === undefined) {
                        lnSpcNode = getTextByPathList(txBodyPPr, ["a:lnSpc", "a:spcPct", "attrs", "val"]);
                        if (lnSpcNode === undefined) {
                            lnSpcNode = getTextByPathList(txBodyPPr, ["a:lnSpc", "a:spcPts", "attrs", "val"]);
                            if (lnSpcNode !== undefined) { lnSpcNodeType = "Pts"; }
                        }
                    }
                }
            }
            ...
            // spcPts values are in 100ths of a point; convert to px (1pt = 4/3 px)
            if (spcBefNode !== undefined) {
                spcBefor = (parseInt(spcBefNode) / 100) * (4 / 3);
            } else if (spcBefPctNode !== undefined) {
                // spcPct for spcBef: percentage of line height — use em units
                spcBefor = parseInt(spcBefPctNode) / 100000; // as fraction; apply below as em
            }
            if (spcAftNode !== undefined) {
                spcAfter = (parseInt(spcAftNode) / 100) * (4 / 3);
            } else if (spcAftPctNode !== undefined) {
                spcAfter = parseInt(spcAftPctNode) / 100000;
            }

            if (lnSpcNode !== undefined) {
                if (lnSpcNodeType == "Pts") {
                    // spcPts is in 100ths of a point; convert to px (1pt = 4/3 px)
                    var lineHeightPx = (parseInt(lnSpcNode) / 100) * (4 / 3);
                    marginTopBottomStr += "line-height: " + lineHeightPx + "px;";
                } else {
                    // spcPct is in 1000ths of a percent; 100000 = 100% = 1.0
                    var fct = parseInt(lnSpcNode) / 100000;
                    // Unitless line-height inherits as a ratio, so child spans compute it
                    // against their own font-size even when parent has font-size:0px
            ...
            if (spcBefor > 0) {
                marginTopBottomStr += "margin-top: " + spcBefor + "px;";
            }
            //}
            if (spcAfter > 0) {
        ...
        function getFontType(node, type, warpObj, pFontStyle) {
            ...
            // Dynamically load any new font via Google Fonts
            if (typeface !== undefined && typeface !== "inherit") {
                if (!window._pptxLoadedFonts) window._pptxLoadedFonts = {};
                if (!window._pptxLoadedFonts[typeface]) {
                    window._pptxLoadedFonts[typeface] = true;
                    var family = typeface.replace(/ /g, "+");
                    var linkId = "pptxjs-font-" + family;
                    if (!document.getElementById(linkId)) {
                        var linkEl = document.createElement("link");
                        linkEl.id = linkId;
                        linkEl.rel = "stylesheet";
                        linkEl.href = "https://fonts.googleapis.com/css2?family=" + family + ":wght@400;700&display=swap";
                        document.head.appendChild(linkEl);
                    }
                }
            }
        ...
        function getBorder(node, pNode, isSvgMode, bType, warpObj) {
            ...
            if (lineNode !== undefined) {
                ...
                if (fillTyp == "NO_FILL") {
                    ...
                    // Resolve phClr hint from lnRef so theme lines using phClr get a real color
                    var lnRefNodeForPhClr = getTextByPathList(node, ["p:style", "a:lnRef"]);
                    var phClrHint;
                    if (lnRefNodeForPhClr !== undefined) {
                        var lnRefClrVal = getTextByPathList(lnRefNodeForPhClr, ["a:schemeClr", "attrs", "val"]);
                        if (lnRefClrVal && lnRefClrVal !== "phClr") {
                            phClrHint = getSchemeColorFromTheme("a:" + lnRefClrVal, undefined, undefined, warpObj);
                        }
                    }
                    borderColor = getSolidFill(lineNode["a:solidFill"], undefined, phClrHint, warpObj);
            ...
            // 2. drawingML namespace - resolve color from lnRef style reference
            if (borderColor === undefined) {
                var lnRefNode = getTextByPathList(node, ["p:style", "a:lnRef"]);
                if (lnRefNode !== undefined) {
                    var lnRefSchemeClrVal = getTextByPathList(lnRefNode, ["a:schemeClr", "attrs", "val"]);
                    if (lnRefSchemeClrVal === "phClr") {
                        // phClr on lnRef cannot resolve without external context; fall back to theme dk1
                        borderColor = getSchemeColorFromTheme("a:dk1", undefined, undefined, warpObj);
                    } else {
        ...
        function getBackground(warpObj, slideSize, index) {
            ...
            if (index == 13) {
                console.log("[SLIDE 13 BG HTML LENGTH]", result.length);
                // Extract just the inline styles and SVG fill colors
                var styleMatches = result.match(/style='[^']*'/g) || [];
                var fillMatches = result.match(/fill="[^"]*"/g) || [];
                console.log("[SLIDE 13 BG STYLES]", JSON.stringify(styleMatches));
                console.log("[SLIDE 13 BG SVG FILLS]", JSON.stringify(fillMatches));
            }
            return result;

        }
        function resolveFillFromStyleList(listName, trueIdx, phClr, clrMapOvr, slideMasterContent, warpObj, sorce, index) {
            var fillLst = warpObj["themeContent"]["a:theme"]["a:themeElements"]["a:fmtScheme"][listName];
            console.log("[resolveFillFromStyleList] listName=", listName, "trueIdx=", trueIdx, "phClr=", phClr, "sorce=", sorce, "fillLst=", fillLst);
            if (fillLst === undefined) return undefined;
            var sortblAry = [];
            Object.keys(fillLst).forEach(function (key) {
                var fillLstTyp = fillLst[key];
                if (key != "attrs") {
                    if (fillLstTyp.constructor === Array) {
                        for (var i = 0; i < fillLstTyp.length; i++) {
                            var obj = {};
                            obj[key] = fillLstTyp[i];
                            obj["idex"] = fillLstTyp[i]["attrs"]["order"];
                            obj["attrs"] = { "order": fillLstTyp[i]["attrs"]["order"] };
                            sortblAry.push(obj);
                        }
                    } else {
                        var obj = {};
                        obj[key] = fillLstTyp;
                        obj["idex"] = fillLstTyp["attrs"]["order"];
                        obj["attrs"] = { "order": fillLstTyp["attrs"]["order"] };
                        sortblAry.push(obj);
                    }
                }
            });
            sortblAry.sort(function (a, b) { return a.idex - b.idex; });
            var fillEntry = sortblAry[trueIdx - 1];
            console.log("[resolveFillFromStyleList] fillEntry=", fillEntry);
            if (fillEntry === undefined) return undefined;
            var fillTyp = getFillType(fillEntry);
            console.log("[resolveFillFromStyleList] fillTyp=", fillTyp);
            if (fillTyp == "SOLID_FILL") {
                var sldFill = fillEntry["a:solidFill"];
                var sldBgClr = getSolidFill(sldFill, clrMapOvr, phClr, warpObj);
                console.log("[resolveFillFromStyleList] SOLID_FILL result=#" + sldBgClr);
                return "background: #" + sldBgClr + ";";
            } else if (fillTyp == "GRADIENT_FILL") {
                var result = getBgGradientFill(fillEntry, phClr, slideMasterContent, warpObj);
                console.log("[resolveFillFromStyleList] GRADIENT_FILL result=", result);
                return result;
            } else if (fillTyp == "PIC_FILL") {
                return getBgPicFill(fillEntry, sorce || "themeBg", warpObj, phClr, index);
            }
            return undefined;
        }
        function getSlideBackgroundFill(warpObj, index) {
            ...
            var bgPrLayout = getTextByPathList(slideLayoutContent, ["p:sldLayout", "p:cSld", "p:bg", "p:bgPr"]);
            var bgRefLayout = getTextByPathList(slideLayoutContent, ["p:sldLayout", "p:cSld", "p:bg", "p:bgRef"]);
            var bgPrMaster = getTextByPathList(slideMasterContent, ["p:sldMaster", "p:cSld", "p:bg", "p:bgPr"]);
            var bgRefMaster = getTextByPathList(slideMasterContent, ["p:sldMaster", "p:cSld", "p:bg", "p:bgRef"]);
            console.log("=== SLIDE " + index + " BG DEBUG === " + JSON.stringify({
                slide_bgPr: bgPr !== undefined ? Object.keys(bgPr) : null,
                slide_bgRef: bgRef !== undefined ? bgRef : null,
                layout_bgPr: bgPrLayout !== undefined ? Object.keys(bgPrLayout) : null,
                layout_bgRef: bgRefLayout !== undefined ? bgRefLayout : null,
                master_bgPr: bgPrMaster !== undefined ? Object.keys(bgPrMaster) : null,
                master_bgRef: bgRefMaster !== undefined ? bgRefMaster : null
            }));
            var clrScheme = getTextByPathList(warpObj["themeContent"], ["a:theme", "a:themeElements", "a:clrScheme"]);
            if (clrScheme) {
                var themeColors = {};
                ["a:dk1","a:dk2","a:lt1","a:lt2","a:accent1","a:accent2","a:accent3","a:accent4","a:accent5","a:accent6"].forEach(function(k) {
                    var n = clrScheme[k];
                    if (n) themeColors[k] = getTextByPathList(n, ["a:srgbClr","attrs","val"]) || getTextByPathList(n, ["a:sysClr","attrs","lastClr"]);
                });
                console.log("=== SLIDE " + index + " THEME COLORS === " + JSON.stringify(themeColors));
            }
            ...
            if (bgPr !== undefined) {
                ...
                console.log("[SLIDE BG] bgRef=", bgRef, "phClr=", phClr, "idx=", idx, "clrMapOvr=", clrMapOvr);


                if (idx == 0 || idx == 1000) {
                    //no background
                } else if (idx > 0 && idx < 1000) {
                    bgcolor = resolveFillFromStyleList("a:fillStyleLst", idx, phClr, clrMapOvr, slideMasterContent, warpObj, "slideBg", index);
                } else if (idx > 1000) {
                    var trueIdx = idx - 1000;
                    bgcolor = resolveFillFromStyleList("a:bgFillStyleLst", trueIdx, phClr, clrMapOvr, slideMasterContent, warpObj, "slideBg", index);
                }
                ...
                if (bgPr !== undefined) {
                    ...
                    if (bgFillTyp == "SOLID_FILL") {
                        var sldFill = bgPr["a:solidFill"];
                        console.log("[LAYOUT BG bgPr] solidFill=", JSON.stringify(sldFill), "clrMapOvr=", JSON.stringify(clrMapOvr));
                        var sldBgClr = getSolidFill(sldFill, clrMapOvr, undefined, warpObj);
                        console.log("[LAYOUT BG bgPr] resolved=#" + sldBgClr);
                        bgcolor = "background: #" + sldBgClr + ";";
                    } else if (bgFillTyp == "GRADIENT_FILL") {
                        bgcolor = getBgGradientFill(bgPr, undefined, slideMasterContent, warpObj);
                    } else if (bgFillTyp == "PIC_FILL") {
                    ...
                    var phClr = getSolidFill(bgRef, clrMapOvr, undefined, warpObj);
                    var idx = Number(bgRef["attrs"]["idx"]);
                    console.log("[LAYOUT BG] bgRef=", bgRef, "phClr=", phClr, "idx=", idx, "clrMapOvr=", clrMapOvr);

                    if (idx == 0 || idx == 1000) {
                        //no background
                    } else if (idx > 0 && idx < 1000) {
                        bgcolor = resolveFillFromStyleList("a:fillStyleLst", idx, phClr, clrMapOvr, slideMasterContent, warpObj, "slideLayoutBg", index);
                    } else if (idx > 1000) {
                        var trueIdx = idx - 1000;
                        bgcolor = resolveFillFromStyleList("a:bgFillStyleLst", trueIdx, phClr, clrMapOvr, slideMasterContent, warpObj, "slideLayoutBg", index);
                    }
                } else {
                    ...
                    if (bgPr !== undefined) {
                        ...
                        var idx = Number(bgRef["attrs"]["idx"]);
                        console.log("[MASTER BG] bgRef=", bgRef, "phClr=", phClr, "idx=", idx, "clrMap=", clrMap);

                        if (idx == 0 || idx == 1000) {
                            //no background
                        } else if (idx > 0 && idx < 1000) {
                            bgcolor = resolveFillFromStyleList("a:fillStyleLst", idx, phClr, clrMap, slideMasterContent, warpObj, "slideMasterBg", index);
                        } else if (idx > 1000) {
                            var trueIdx = idx - 1000;
                            bgcolor = resolveFillFromStyleList("a:bgFillStyleLst", trueIdx, phClr, clrMap, slideMasterContent, warpObj, "slideMasterBg", index);
                        }
                    }
                }
            }

            console.log("=== SLIDE " + index + " RESULT === " + bgcolor);
            return bgcolor;
        }
        ...
        function getShapeFill(node, pNode, isSvgMode, warpObj, source) {
            ...
            var fillColor;
            var _debugBg = (source && source.indexOf("Bg") !== -1);
            if (_debugBg) {
                var _shapeName = getTextByPathList(node, ["p:nvSpPr", "p:cNvPr", "attrs", "name"]) || "unknown";
                var _fillRef = getTextByPathList(node, ["p:style", "a:fillRef"]);
                var _fillRefIdx = _fillRef ? getTextByPathList(_fillRef, ["attrs", "idx"]) : "none";
                console.log("[SHAPE FILL BG] source=" + source + " shape=" + _shapeName + " fillType=" + fillType + " fillRefIdx=" + _fillRefIdx + " isSvg=" + isSvgMode);
            }
            if (fillType == "NO_FILL") {
                ...
                if (_debugBg) console.log("[SHAPE FILL BG] SOLID resolved=" + fillColor + " shpFill=", JSON.stringify(shpFill));
            } else if (fillType == "GRADIENT_FILL") {
                var shpFill = node["p:spPr"]["a:gradFill"];
                fillColor = getGradientFill(shpFill, warpObj);
                if (_debugBg) console.log("[SHAPE FILL BG] GRADIENT resolved=", fillColor);
            ...
            if (fillColor === undefined) {
                // Placeholder shapes (title, body, subtitle etc.) do NOT use the
                // p:style/a:fillRef theme fill — they are always transparent unless
                // p:spPr explicitly sets a fill. Applying the style fill produces
                // unwanted coloured rectangles behind the text.
                var isPlaceholder = getTextByPathList(node, ["p:nvSpPr", "p:nvPr", "p:ph"]) !== undefined;
                if (isPlaceholder && fillType === "") {
                ...
                if (isNaN(idx) || idx == 0 || idx == 1000) {
                    // no fill (idx 0 = no fill, idx 1000 = background fill slot 0 = also no fill)
                    return isSvgMode ? "none" : "";
                } else {
                    // phClr is the accent color embedded in the fillRef node
                    var phClr = getSolidFill(clrName, undefined, undefined, warpObj);
                    if (_debugBg) console.log("[SHAPE FILL BG] phClr=" + phClr + " idx=" + idx);
                    var fmtScheme = getTextByPathList(warpObj["themeContent"], ["a:theme", "a:themeElements", "a:fmtScheme"]);
                    var fillStyleLst, trueIdx;
                    if (idx < 1000) {
                        fillStyleLst = fmtScheme !== undefined ? fmtScheme["a:fillStyleLst"] : undefined;
                        trueIdx = idx - 1;
                    } else {
                        fillStyleLst = fmtScheme !== undefined ? fmtScheme["a:bgFillStyleLst"] : undefined;
                        trueIdx = idx - 1001;
                    }
                    if (fillStyleLst !== undefined) {
                        // Collect fills in order
                        var sortblAry = [];
                        Object.keys(fillStyleLst).forEach(function (key) {
                            if (key === "attrs") return;
                            var entry = fillStyleLst[key];
                            if (Array.isArray(entry)) {
                                for (var i = 0; i < entry.length; i++) {
                                    var o = {}; o[key] = entry[i];
                                    o["idex"] = parseInt(getTextByPathList(entry[i], ["attrs", "order"])) || sortblAry.length;
                                    sortblAry.push(o);
                                }
                            } else {
                                var o = {}; o[key] = entry;
                                o["idex"] = parseInt(getTextByPathList(entry, ["attrs", "order"])) || sortblAry.length;
                                sortblAry.push(o);
                            }
                        });
                        sortblAry.sort(function (a, b) { return a.idex - b.idex; });
                        var themeEntry = sortblAry[trueIdx];
                        if (themeEntry !== undefined) {
                            var tFillTyp = getFillType(themeEntry);
                            if (_debugBg) console.log("[SHAPE FILL BG] themeEntry tFillTyp=" + tFillTyp + " trueIdx=" + trueIdx, themeEntry);
                            if (tFillTyp == "NO_FILL") {
                                return isSvgMode ? "none" : "";
                            } else if (tFillTyp == "SOLID_FILL") {
                                fillColor = getSolidFill(themeEntry["a:solidFill"], undefined, phClr, warpObj);
                                if (_debugBg) console.log("[SHAPE FILL BG] SOLID_FILL result=" + fillColor);
                            } else if (tFillTyp == "GRADIENT_FILL") {
                                var gradResult = getGradientFill(themeEntry["a:gradFill"], warpObj, phClr);
                                if (_debugBg) console.log("[SHAPE FILL BG] GRADIENT_FILL result=", gradResult);
                                if (gradResult !== undefined) fillColor = gradResult;
                            }
                        }
                    }
                    // Do NOT fall back to phClr as a fill — phClr is an accent hint for
                    // theme fills, not a shape background. If the theme entry is undefined
                    // or unrecognised, leave fillColor undefined so the shape stays transparent.
                }
            }
            // 3. is group fill
            if (fillColor === undefined) {
        ...
        function getGradientFill(node, warpObj, phClr) {
            ...
            for (var i = 0; i < gsLst.length; i++) {
                var lo_tint;
                var lo_color = getSolidFill(gsLst[i], undefined, phClr, warpObj);
                //console.log("lo_color",lo_color)
                color_ary[i] = lo_color;
            }
            //get rot
        ...
        function IsVideoLink(vdoFile) {
            // Simple check: any http/https/ftp URL is a link, not an embedded file
            return /^(https?|ftp):\/\//i.test(vdoFile);
        }

        function toEmbedUrl(url) {
            // Convert YouTube watch/short URLs to embeddable URLs
            var ytWatch = url.match(/(?:youtube\.com\/watch\?(?:.*&)?v=|youtu\.be\/)([a-zA-Z0-9_-]{11})/);
            if (ytWatch) return "https://www.youtube.com/embed/" + ytWatch[1] + "?autoplay=0";
            // Vimeo: https://vimeo.com/12345678 → https://player.vimeo.com/video/12345678
            var vimeo = url.match(/vimeo\.com\/(\d+)/);
            if (vimeo) return "https://player.vimeo.com/video/" + vimeo[1];
            // Already an embed URL or unknown provider — use as-is
            return url;
        }
